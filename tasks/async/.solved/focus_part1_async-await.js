...
async function run () {
    ...
    const requisites = await sendRequest(`${API.orgReqs}?ogrn=${orgOgrns.join(',')}`);
    const orgsMap = reqsToMap(requisites);

    const analytics = await sendRequest(`${API.analytics}?ogrn=${orgOgrns.join(',')}`);
    addInOrgsMap(orgsMap, analytics, 'analytics');

    const buh = await sendRequest(`${API.buhForms}?ogrn=${orgOgrns.join(',')}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = function() {
            if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                ...
                return reject(`${xhr.status}: ${xhr.message}`);
            }
        };
