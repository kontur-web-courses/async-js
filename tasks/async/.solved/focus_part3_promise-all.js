...
async function run () {
    ...
    requests.push(sendRequest(`${API.orgReqs}?ogrn=${orgOgrns.join(',')}`));
    requests.push(sendRequest(`${API.analytics}?ogrn=${orgOgrns.join(',')}`));
    requests.push(sendRequest(`${API.buhForms}?ogrn=${orgOgrns.join(',')}`));
